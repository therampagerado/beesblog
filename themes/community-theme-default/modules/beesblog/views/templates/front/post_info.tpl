<div class="beesblog-post-info">
    ...
    {if isset($showDate) && $showDate}
        ...
        <time>{$post->published|date_format}</time>
    {/if}
    <i class="icon icon-object-group"></i>&nbsp;
    <span><a href="{$post->category->link}">{$post->category->title|escape:'htmlall':'UTF-8'}</a></span>
    {if isset($showComments) && $showComments && $post->comments_enabled}
        <span class="beesblog-comment-counter">
            <i class="icon icon-comments"></i>&nbsp;
            <a title="{l s='0 Comments' mod='beesblog'}" href="{$postPath|escape:'htmlall':'UTF-8'}#disqus_thread" data-disqus-identifier="{$post->id|intval}">{l s='0 Comments' mod='beesblog'}</a>
        </span>
    {/if}
